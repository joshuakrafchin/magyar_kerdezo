import { getState, setState, initState, resetState, exportState, importState, flushState, DEFAULT_INTERVIEW_TOPICS } from './state.js';
import { getMe, logout, generateBatch, extractVocabulary as apiExtractVocab, getInvitations, inviteUser } from './api.js';
import { LEVELS, QUESTIONS_PER_LEVEL, getLevelIndex, TOPICS } from './curriculum.js';
...
function showScreen(name) {
  ...
  nav.classList.toggle('hidden', name === 'login' || name === 'welcome' || name === 'loading');
    ...
    for (let i = startLevel; i < LEVELS.length; i++) {
      ...
      for (let b = 0; b < count; b += batchSize) {
        ...
        for (let j = 0; j < batchCount; j++) {
          batchTopics.push(curriculumTopics[(b + j) % curriculumTopics.length]);
        }

        const { questions, vocabulary } = await generateBatch(
          level, state.aboutMeEssay, state.interviewTopics, batchTopics, batchCount
        );

        // Add IDs and repetition data
        const mapped = questions.map((q, qi) => ({
          id: `${level}-${Date.now()}-${qi}`,
          level,
          ...q,
          repetition: q.repetition || initRepetitionData(),
        }));
        ...
        if (b + batchSize < count) await sleep(1000);
      }
    }
...
function initSettings() {
  ...
  // Account info
  if (currentUser) {
    document.getElementById('account-info').textContent = `Signed in as ${currentUser.name || currentUser.email}`;
  }
...
document.getElementById('btn-extract-vocab').onclick = async () => {
  ...
  try {
    ...
    const words = await apiExtractVocab(sentenceArr);
...
async function loadInvitations() {
  try {
    ...
    list.forEach(inv => {
      ...
      row.textContent = `${inv.email} — invited ${new Date(inv.created_at).toLocaleDateString()}`;
      container.appendChild(row);
    });
  } catch { /* ignore */ }
}
...
document.getElementById('btn-invite').onclick = async () => {
  ...
  try {
    ...
    status.textContent = `Invited ${email}!`;
...
document.getElementById('btn-dropdown-logout').onclick = () => logout();

// ─── User Menu Dropdown ───
document.getElementById('btn-user-menu').onclick = (e) => {
  e.stopPropagation();
...
// ─── Page unload — flush state ───
window.addEventListener('beforeunload', () => flushState());

// ─── Init ───
async function init() {
  // Check login error in URL
  const params = new URLSearchParams(window.location.search);
  if (params.get('error') === 'not_invited') {
    const errEl = document.getElementById('login-error');
    errEl.textContent = `${params.get('email') || 'Your account'} has not been invited. Ask the admin for access.`;
  ...
  // Set avatar
  const avatarBtn = document.getElementById('btn-user-menu');
  if (currentUser.picture) {
    avatarBtn.style.backgroundImage = `url(${currentUser.picture})`;
    avatarBtn.style.backgroundSize = 'cover';
    avatarBtn.textContent = '';
  } else {
    avatarBtn.textContent = currentUser.name?.[0] || '?';
  }

  const ddInfo = document.getElementById('user-dropdown-info');
  ddInfo.textContent = currentUser.name || currentUser.email;
